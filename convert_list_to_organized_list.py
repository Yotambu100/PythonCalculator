from minus_organized_helper import MinusOrganized
...
class ConvertListToOrganizedList:
    ...
    def create_organize_list(self):
        ...
        self.organized_list = self._handle_minus()

        # for every chained left to right operator call(except minus)
    ...
    def _handle_operator(self, left_chained_operator, current_operator_char,
        ...
        if current_operator_char == left_chained_operator:
            ...
            if current_operator.operator_power > self. \
                    OPERATORS_DICTIONARY[left_chained_operator].operator_power:
    ...
    def _handle_minus(self):
        """
        The function handle minus chaining by adding open bracket
        and zero before the operator and close
        bracket after at least one number and before operator(minus is
        a special case because unlike other chaining operators minus
        has always the highest power)
        """
        # initializing the new organized list
        new_list = []
        ...
        for index_of_char in range(0, len(equation_list)):
            ...
            # if number add the number to the list
            # and add that a number was found in current depth of bracket
            # (only if needed)
            if isinstance(current_char, float):
                new_list.append(current_char)
                self._number_found_in_current_bracket_minus(
                    current_number_of_bracket)

            # if current char is open bracket increase the current
            # number in bracket and add to list
            elif current_char == '(':
                current_number_of_bracket += 1
                new_list.append(current_char)
            ...
            # brackets)
                ...
                self._pop_current_bracket(new_list, current_number_of_bracket)
                current_number_of_bracket -= 1
                new_list.append(current_char)
                self._number_found_in_current_bracket_minus(
                    current_number_of_bracket)
                ...
                # if got to here current char must be operator
                # call _handle_operator_minus() to manage the operators
                self._handle_operators_minus(current_char, new_list,
                                             index_of_char,
                                             equation_list,
                                             current_number_of_bracket)
    ...
    def _add_close_bracket_if_needed_minus(self, current_number_of_bracket,
                                           new_list, current_operator_power):
        """
        The function add close bracket if needed(number was found and
        current operator power is equal or smaller)
        :param current_number_of_bracket:The depth in the unclosed
        brackets
        :param new_list:The new organized list
        :param current_operator_power:The power of the current
        operator
        """

        # if both stacks are not empty and the top of bracket stack
        # current number in bracket is
        # equal to the top of number_in_bracket_stack (meaning there
        # was a number in the same depth as an open waiting to be
        # closed bracket) and the top of number_in_bracket_stack is equal
        # to the current_number_of_bracket (meaning the open unclosed
        # bracket is at the same depth as current depth)
        # pop the number_in_bracket_stack to remove the number in the
        # same depth and until the top of bracket_stack is not the same
        # as current depth pop from bracket_stack and add close bracket
        # to the new organized list
        if self.bracket_stack and self.number_in_bracket_stack \
                and self.bracket_stack[-1] \
                .current_number_in_bracket == current_number_of_bracket \
                and self.bracket_stack[-1] \
                .power_before >= current_operator_power \
                and self.number_in_bracket_stack[-1] \
                == current_number_of_bracket:

            self.number_in_bracket_stack.pop()

            while self.bracket_stack \
                    and self.bracket_stack[-1].current_number_in_bracket \
                    == current_number_of_bracket \
                    and self.bracket_stack[
                -1].power_before >= current_operator_power:
                new_list.append(')')
                self.bracket_stack.pop()

    def _handle_operators_minus(self, current_operator, new_list,
                                operator_index,
                                original_list, current_number_of_bracket):
        """
        The function analyzed the operator based of his type and his
        location and determine what to do with it
        ...
        # if the operator is at the start of the equation or after
        # open bracket
        if operator_index == 0 or original_list[operator_index - 1] == '(':

            # if minus add zero before and continue
            if current_operator == '-':
                new_list.append(0.0)
                new_list.append(current_operator)
            else:
                new_list.append(current_operator)

        # if the operator is a chaining minus(meaning before him there is an
        # operator that is not right of number)
        elif current_operator == '-' and original_list[
            operator_index - 1] in self.OPERATORS_DICTIONARY \
                and not self.OPERATORS_DICTIONARY[original_list[
            operator_index - 1]].operator_position == OperatorPosition.RIGHT_OF_NUMBER:

            # add "(0-" to the new organized list
            # and save the operator before the minus power and current
            # depth in bracket in current_number_in_bracket
            new_list.append('(')
            new_list.append(0.0)
            new_list.append(current_operator)

            minus_power_depth = MinusOrganized()

            # save the current depth in bracket
            minus_power_depth.current_number_in_bracket = \
                current_number_of_bracket

            # save the power of the operator before the minus
            minus_power_depth.power_before = \
                self.OPERATORS_DICTIONARY[original_list[operator_index - 1]] \
                    .operator_power

            self.bracket_stack.append(minus_power_depth)
        else:
            # else the operator is regular operator(every operator
            # include regular minus but not sign minus)
            # so check if need to add close brackets before him
            # and add the operator
            self._add_close_bracket_if_needed_minus(
                current_number_of_bracket, new_list,
                self.OPERATORS_DICTIONARY[current_operator].operator_power)
            new_list.append(current_operator)

    def _pop_current_bracket(self, new_list, current_number_of_bracket):
        """
        The function pop all the waiting closed bracket into the
        equation at the same depth of bracket as given
        :param new_list:The new organized list
        :param current_number_of_bracket:The depth of current number
        in bracket
        """
        while self.bracket_stack and self.bracket_stack[-1] \
                .current_number_in_bracket == current_number_of_bracket:
    ...
    def _number_found_in_current_bracket_minus(self,
                                               current_number_of_bracket):
        """
        The function add that a number was found
        in current_number_of_bracket only if there is need(meaning the
        ...
        if self.bracket_stack:
            ...
            # stack of number_in_bracket_stack)
            if self.bracket_stack[
                -1].current_number_in_bracket == current_number_of_bracket \
                    and (not self.number_in_bracket_stack
                         or self.number_in_bracket_stack[
                             -1] != current_number_of_bracket):
                self.number_in_bracket_stack.append(current_number_of_bracket)
